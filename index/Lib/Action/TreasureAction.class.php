...
class TreasureAction extends BaseAction {
	...
	public function index()
	{
		...
		$leftlist = $classNameModel->where(array("pid"=>3))->order("sort desc")->select();
		
		if($id){
			$classname = $classNameModel->where(array("id"=>$id))->find();
			$this->assign("id",$id);
		}else{
			$classname = $classNameModel->where(array("pid"=>3))->order("sort desc")->limit(1)->find();
			$id = $classname["id"];
		...
		$this->assign("left_link","/treasure/index/");
		$this->assign("list_link","/treasure/detail/");
