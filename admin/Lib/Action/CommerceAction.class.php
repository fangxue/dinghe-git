...
class CommerceAction extends BaseAction {
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/admin.php/Commerce/index');
				}
			}else{
	...
	public function del()
	{
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/admin.php/Commerce/index");
		}
	...
	*/
	...
	public function addArticle(){
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/admin.php/Commerce/articleManage/id/'.$data["classid"]);
				}
			}else{
				$result = $articleModel->where('id='.$data["id"])->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/admin.php/Commerce/articleManage/id/'.$data["classid"]);
				}
			}
	...
	public function delarticle(){
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/admin.php/Commerce/articleManage/id/".$id);
		}
