...
class TreasureAction extends BaseAction {
	public function index(){
		...
		$list = $classNameModel->where(array("pid"=>3))->order("create_time desc")->select();
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Treasure/index');
				}
			}else{
				$result = $classNameModel->where('id='.$data["id"])->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/Treasure/index');
				}
			}
	...
	public function del()
	{
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Treasure/index");
		}
	...
	public function addArticle(){
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Treasure/articleManage/id/'.$data["classid"]);
				}
			}else{
				$result = $articleModel->where('id='.$data["id"])->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/Treasure/articleManage/id/'.$data["classid"]);
				}
			}
	...
	public function delarticle(){
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Treasure/articleManage/id/".$id);
		}
