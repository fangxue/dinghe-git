...
class PropertyAction extends BaseAction {
	public function index(){
		...
		$count = $classNameModel->where(array("pid"=>2))->order("create_time desc")->count();
		$page = new Page($count,10);

		$list = $classNameModel->where(array("pid"=>2))->order("create_time desc")->limit($page->firstRow.','.$page->listRows)->select();
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Property/index');
				}
			}else{
	...
	public function del()
	{
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Property/index");
		}
		$articleModel = M("article");
	...
	public function addArticle(){
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Property/articleManage/id/'.$data["classid"]);
				}
			}else{
				$result = $articleModel->where('id='.$data["id"])->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/Property/articleManage/id/'.$data["classid"]);
				}
			}
	...
	public function delarticle(){
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Property/articleManage/id/".$id);
		}
