...
class DownloadAction extends BaseAction {
	...
	public function addTable(){
		...
		if($this->getParam('post')){
			...
			if (!empty($_FILES)&&$_FILES['Filedata']&&$_FILES['Filedata']['name']) {
				...
				$savePath = '../wwwroot/Uploads/downloadTable/';
				...
				$info["table_link"] = "/Uploads/downloadTable/".$uploadList[0]['savename'];
			}
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Download/index');
				}
			}else{
	...
	public function del()
	{
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Download/index");
		}
