...
class ProgramAction extends BaseAction {
	...
	public function addClass(){
		...
		if($this->getParam('post')){
			...
			if (!empty($_FILES)&&$_FILES['Filedata']&&$_FILES['Filedata']['name']) {
				$res = $this->_upload($files,"/wwwroot/Uploads/program/",'/Uploads/program/');
				$info["img"] = $res;
			}
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Program/index');
				}
			}else{
				$result = $classNameModel->where(array("id"=>$data['id']))->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/Program/index');
				}
			}
	...
	public function del()
	{
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Program/index");
		}
	...
	public function addArticle(){
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/Program/articleManage/id/'.$data["classid"]);
				}
			}else{
				$result = $articleModel->where('id='.$data["id"])->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/Program/articleManage/id/'.$data["classid"]);
				}
			}
	...
	public function delarticle(){
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/Program/articleManage/id/".$id);
		}
