...
class DingheAction extends BaseAction {
    ...
    public function uploadImage(){
		...
		if (!empty($_FILES)&&$_FILES['Filedata']&&$_FILES['Filedata']['name']) {
			$res = $this->_upload($files,"/public/Uploads/dinghe/",'/dinghe/');
			$imageModel = M("image");
			$info["path"] = $res;
	...
	public function del()
	{
		...
		if(empty($info)){
			$this->error("此条记录不存在，或已被删除","/admin.php/Dinghe/index");
		}
		...
		if($this->getParam('post')){
			...
			if($data["id"] == ''){
				...
				if(is_numeric($result)){
					$this->success('添加成功','/admin.php/Dinghe/index');
				}
			}else{
				$result = $aboutModel->where(array("id"=>$data['id']))->save($info);
				if(is_numeric($result)){
					$this->success('修改成功','/admin.php/Dinghe/index');
				}
			}
