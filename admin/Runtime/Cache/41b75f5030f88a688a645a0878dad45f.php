	...
	<table class="table table-hover folder-list" style="margin-top:10px">
		<tbody>
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php echo (date('Y-m-d',$info["last_date"])); ?></td>
					<!-- <td>
						<a class="btn btn-default btn-xs" href="/admin.php/Public/addUser/id/<?php echo ($info["id"]); ?>">编辑</a>
						<a class="btn btn-default btn-xs" href="/admin.php/Public/del/id/<?php echo ($info["id"]); ?>">删除</a>
					</td> -->
